...
use Code16\Sharp\Utils\Fields\FieldIdentifier;
...
trait HandlesFieldContainer
{
    ...
    {
        $identifier = FieldIdentifier::decrypt(request()->query('identifier'));

        if ($identifier->isEmbed()) {
            return $this->getEmbedFromKey($identifier->data['embedKey']);
        }

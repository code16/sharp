...
import { h, MaybeRef, render, unref } from "vue"
...
export function componentToString<P>(config: MaybeRef<ChartConfig>, component: Constructor<P>, props?: P) {
  if (!isClient)
    return
  ...
  return (_data: any, x: number | Date) => {
    ...
    const vnode = h<unknown>(component, { ...props, payload: data, config: unref(config), x })

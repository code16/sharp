<script setup lang="ts">
    ...
    function onSortClick(fieldKey: string) {
        const dir = props.entityList.currentSort === fieldKey
            ? props.entityList.currentSortDir === 'desc' ? null : 'desc'
            : 'asc';
        emit('update:query', {
            ...props.entityList.query,
            page: 1,
    ...
    <WithCommands :commands="commands">
        ...
        <template v-if="entityList">
            <template v-if="showSearchField && entityList.config.searchable || entityList.visibleFilters?.length">
                <StickyTop class="group container px-4 lg:px-6 flex gap-3 mb-4 pointer-events-none"
                    :class="cn(
                        'z-30',
                ...
                >
                    <template v-if="showSearchField && entityList.config.searchable">
                        <div class="pointer-events-auto" v-show="!reordering && !selecting && !stuck">
                            <EntityListSearch
                    ...
                    <template v-if="entityList.visibleFilters?.length">
                        <div class="pointer-events-auto min-w-0 overflow-hidden" v-show="!reordering && !selecting && !stuck">
                            ...
                            <div class="relative flex-1 hidden lg:block">
                                ...
                                <div class="hidden md:flex group-data-[wrapping=true]:flex-wrap gap-3"
                                    :class="{
        ...
        <div class="container px-4 lg:px-6">
            <div class="@container">
                <Card class="rounded-none border-x-0 -mx-4 lg:-mx-6 @5xl:!mx-0 @5xl:border @5xl:rounded-lg" :class="reordering ? 'relative z-[11]' : ''">
                    ...
                    <template v-if="entityList">
                        <CardContent :class="entityList.count > 0 ? 'pb-2 px-0' : ''" v-show="!collapsed">
                            <template v-if="entityList.data?.length > 0">
                                <ScrollArea class="w-full" type="auto">
                                    <Table no-scroll class="w-max min-w-full max-w-[768px] md:max-w-[1024px] @5xl:w-full @5xl:max-w-none">
                                        <TableHeader>
                                            <TableRow class="hover:bg-transparent lg:first:*:pl-6 lg:last:*:pr-6">
                                                ...
                                                <template v-for="(field, fieldIndex) in visibleFields">
                                                    <TableHead
...
<!--                                                            </DropdownMenu>-->
                                            ...
                                            <template v-for="(item, itemIndex) in reorderedItems ?? entityList.data">
                                                ...
                                                >
                                                    ...
                                                    <template v-if="!reordering && !selecting && entityList.instanceHasActions(item, showEntityState)">
                                                        <TableCell class="sticky bg-background !pl-1 !pr-4 -right-3 z-10 @5xl:!pl-4 @5xl:!pr-6 @5xl:bg-transparent">
                                                            <div class="absolute pointer-events-none inset-0 shadow-xl border-border transition-opacity group-data-[scroll-arrived-right=true]/viewport:opacity-0 @5xl:opacity-0" aria-hidden="true"></div>
                                                            <DropdownMenu>
                                                                <DropdownMenuTrigger as-child>
                                    ...
                                    <UseWindowSize v-slot="{ height }">
                                        <UseElementBounding class="absolute inset-0 pointer-events-none" v-slot="{ bottom }">
                                            <ScrollBar
                                                class="z-20 bg-background pointer-events-auto"

...
class SelectFormatterTest extends SharpTestCase
{
    ...
    /** @test */
    function we_can_format_a_multiple_object_value_to_front()
    {
        $formatter = new SelectFormatter;
        $field = SharpFormSelectField::make("select", $this->getSelectData())
            ->setMultiple();
    ...
    /** @test */
    function we_can_format_a_multiple_object_with_toArray_value_to_front()
    {
        $formatter = new SelectFormatter;
        $field = SharpFormSelectField::make("select", $this->getSelectData())
            ->setMultiple();
        ...
        $this->assertEquals([1,2], $formatter->toFront($field, [
            new class { function toArray() { return ["id"=>1, "label"=>"A"]; } },
            new class { function toArray() { return ["id"=>2, "label"=>"B"]; } }
        ]));
    }
    ...
    /** @test */
    function we_can_define_idAttribute_for_a_multiple_array_value_from_front()
    {
        $formatter = new SelectFormatter;
        $field = SharpFormSelectField::make("select", $this->getSelectData())
            ->setMultiple()

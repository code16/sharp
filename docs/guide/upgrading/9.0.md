...
To migrate, you should first create (if needed) a Service Provider which extends `Code16\Sharp\SharpAppServiceProvider` and implements the `configureSharp()` method:
...
Note: be sure to [register this new Service Provider](https://laravel.com/docs/providers#registering-providers) in your app.

All previous available config keys that are still in use in 9.x have a corresponding method in the `SharpConfigBuilder` class.

## New middleware to declare

Due to migration to inertia, two middleware must be added to the config. 

Note: if you migrated to the new config builder class, you should be ok unless you have explicitly overridden the whole middleware list.
...
If you are using editor uploads, you will need migrate `<x-sharp-image>` and `<x-sharp-file>` elements in the content. We provide a helper trait to be used as follows :

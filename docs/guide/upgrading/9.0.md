...
## `SharpFormUploadField`’s `setFileFilter()`, `setFileFilterImages()` and `setMaxFileSize()` methods were deprecated because of the new validation system

This isn't a breaking change, but you should migrate to this API to be able to leverage the new validation system. Here's an example:

...
For custom delete confirmations, you should call the new `SharpShow::configureDeleteConfirmationText(string $message)` and `SharpEntityList::configureDelete(bool $allow, string $message)`.

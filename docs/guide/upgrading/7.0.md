...
Replaced by the new `<x-sharp-content>` component: [see dedicated documentation](../display-markdown-content.md).

## Entity classes (SharpEntity)! 
...
Note that **there is a real breaking change with Dashboards**: the permission method to see a Dashboard was renamed to `entity()`, to be consistent:
